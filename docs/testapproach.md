...
## Contents
- [Test Approach Documentation](#test-approach-documentation)
  - [Contents](#contents)
  ...
  - [Test Approach](#test-approach)
    ...
    - [Snapshot Merging/Pruning](#snapshot-mergingpruning)
...
### Snapshot Merging/Pruning

`taskobra` does lazy evaluated, algorithmic pruning based on a sample rate, base, and exponent for the data being pruned.  The primary operation used is a merge of two Snapshots, since lazy evaluation requires that we can successfully prune data without needing to be aware of all of the data points at once.  Unit tests exist to ensure that the merge operation satisfies the associative and identity properties.  Having guaranteed those properties of merge, the pruning algorithm can be simpler and more easily tested.  The pruning algorithm should be deterministic, and if the output of a prune is pruned again, without any changes, it should yield the same result.
