...
class Snapshot(ORMBase):
    ...
    def prune(cls, t0, snapshots: Collection["Snapshot"]):
        snapshot_iterable = reversed(
            sorted(
                filter(
                    lambda snapshot: snapshot.timestamp <= t0, snapshots
                ),
                key=lambda snapshot: snapshot.timestamp
            )
        )
        prunes = []
        snaps = [next(snapshot_iterable)]
        sample_exponent = 1
        sample_rate = snaps[0].sample_rate
        sample_base = snaps[0].sample_base
        ...
        for snapshot in snapshot_iterable:
            if time_slice[0] > snapshot.timestamp >= time_slice[1]:
    ...
    def __repr__(self):
        ...
        return f"<Snapshot({self.timestamp} : {self.sample_rate}*{self.sample_base}^{self.sample_exponent} : {total_metrics})>"
